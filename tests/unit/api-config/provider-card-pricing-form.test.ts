...
describe('provider card pricing form behavior', () => {
  it('allows openai-compatible provider to add llm/image/video/audio', () => {
    expect(getAddableModelTypesForProvider('openai-compatible:oa-1')).toEqual(['llm', 'image', 'video', 'audio'])
  })

  it('shows llm/image/video/audio tabs by default for openai-compatible even with only image models', () => {
    ...
    expect(visible).toEqual(['llm', 'image', 'video', 'audio'])
  })
